...
const ChatBox = ({ socket, gameId, userId, username }: ChatBoxProps) => {
  ...
  return (
    <div className="flex flex-col h-80 bg-white/5 rounded-xl border border-white/10 overflow-hidden">
      ...
      {/* Input bar – compact and mic always visible */}
      <div className="p-2 border-t border-white/10 flex items-center gap-2 bg-white/5">
        <form onSubmit={sendMessage} className="flex-1 flex items-center gap-2 min-w-0">
          <input
            ...
            className="flex-1 min-w-0 bg-white/10 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-primary-400"
          />
          <button
            type="submit"
            className="flex-shrink-0 px-3 py-2 bg-primary-500/30 hover:bg-primary-500/50 rounded-lg text-white text-sm transition"
          >
            Send
          </button>
        </form>

        {/* Mic button – always on the right, never shrinks */}
        <button
          onClick={isRecording ? stopRecording : startRecording}
          className={`flex-shrink-0 px-3 py-2 rounded-lg text-sm transition flex items-center gap-1 ${
            isRecording
