...
export class GamesService {
  ...
  async makeMove(gameId: string, userId: string, dto: MakeMoveDto) {
    ...
    if (status === 'ACTIVE' && gameData.blackId === 'AI') {
      ...
      const depthMap: Record<string, number> = { easy: 2, medium: 4, hard: 6 };
      const depth = depthMap[difficultyKey] || 4;
      ...
      while (
        ...
        safetyCounter++;
        const aiMove = this.aiService.getBestMove(latestBoard, 'black', depth);
        if (!aiMove) break;
